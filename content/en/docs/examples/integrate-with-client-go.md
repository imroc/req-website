...
Log into the container, use `curl 127.0.0.1:80/dump?enable=true` to enable dump, output example:
...
Similarly, after debugging, you can use `curl 127.0.0.1:80/dump?enable=false` to turn off dump.
