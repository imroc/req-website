...
`Req` 的 dump 能力来自其实现的 `Transport` ，而 `client-go` 提供了自定义 `Transport` 的能力，所以我们可以将 `Req` 的 `Transport` 集成进 `client-go` 中，可以使用开关按需动态打开 dump 能力，看到请求所有细节，以实现 K8S API 的调试。
...
登进容器，使用 `curl 127.0.0.1:80/dump?enable=true` 开启 dump，输出示例:

```txt
Dump is enabled
[GIN] 2022/05/24 - 07:31:45 | 200 |      36.288µs |       127.0.0.1 | GET      "/dump?enable=true"
...
HTTP/1.1 200 OK
Audit-Id: cb07b918-1e9d-45f8-9cd4-8f19202d5e30
